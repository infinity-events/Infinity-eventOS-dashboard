...
const filteredTickets=useMemo(()=>{
  ...
  return tickets.filter(ticket=>{
    ...
    const haystack=[name,user?.email,ticket.code,ticket.wristband?.uid,ticket.wristband?.code].filter(Boolean).join(" ").toLowerCase();
...
return <div className="text-white">
  ...
  <div className="grid grid-cols-1 2xl:grid-cols-[0.9fr_1.1fr] gap-6">
    ...
    <section className="bg-[#17181D] border border-white/5 rounded-xl p-5">
      ...
      {ticketsLoading?<p className="text-gray-400">Caricamento...</p>:<div className="overflow-x-auto"><table className="w-full"><thead><tr className="text-gray-400 text-left border-b border-white/10"><th className="pb-3">Nome</th><th>Ticket</th><th>Braccialetto</th><th>Stato</th></tr></thead><tbody>{filteredTickets.map(ticket=><tr key={ticket.id} onClick={()=>setSelectedTicket(ticket)} className="border-b border-white/5 cursor-pointer hover:bg-white/5"><td className="py-4"><p className="font-medium">{ticketName(ticket)}</p><p className="text-sm text-gray-400">{ticket.user?.email||"Dati utente non disponibili"}</p></td><td><span className="font-medium">{ticket.code}</span><p className="text-gray-400 text-sm">{ticket.type} · {formatPrice(ticket.price)}</p></td><td>{ticket.wristband?.uid||ticket.wristband?.code||"-"}</td><td><StatusBadge status={enteredIds.has(ticket.id)}/></td></tr>)}</tbody></table>{filteredTickets.length===0&&<p className="text-gray-400 py-6 text-center">Nessun partecipante trovato</p>}</div>}
    </section>
  </div>
...
function ParticipantDrawer({ticket,logs,entered,onClose}){const user=ticket.user;return <div className="fixed inset-0 z-40 bg-black/70 flex justify-end" onClick={onClose}><aside className="w-full max-w-xl h-full overflow-y-auto bg-[#17181D] border-l border-white/10 p-6" onClick={e=>e.stopPropagation()}><div className="flex justify-between items-start mb-8"><div><p className="text-sm text-purple-300 uppercase tracking-wide">Scheda partecipante</p><h2 className="text-2xl font-bold mt-2">{ticketName(ticket)}</h2><p className="text-gray-400 mt-1">{user?.email||"Email non disponibile"}</p></div><button onClick={onClose} className="p-2 rounded-lg hover:bg-white/10"><X/></button></div><DetailSection title="Informazioni utente"><Detail label="Nome" value={ticketName(ticket)}/><Detail label="Email" value={user?.email}/><Detail label="Telefono" value={user?.phone}/><Detail label="Città" value={user?.city}/></DetailSection><DetailSection title="Ticket"><Detail label="Codice" value={ticket.code}/><Detail label="Tipo" value={ticket.type}/><Detail label="Prezzo" value={formatPrice(ticket.price)}/><Detail label="Stato" value={ticket.status}/><Detail label="Creato" value={formatDate(ticket.createdAt)}/></DetailSection><DetailSection title="Braccialetto"><Detail label="Stato" value={ticket.wristband?(ticket.wristband.activated?"Attivato":"Non attivato"):"Non associato"}/><Detail label="Codice" value={ticket.wristband?.code}/><Detail label="UID" value={ticket.wristband?.uid}/></DetailSection><DetailSection title="Ingresso"><Detail label="Stato" value={entered?"Entrato":"Da entrare"}/><Detail label="Ultimo ingresso" value={formatDate(logs[0]?.createdAt)}/>{logs.length?<div className="mt-4 space-y-2">{logs.map(log=><div key={log.id} className="rounded-lg bg-black/20 p-3 flex justify-between"><span>{log.method} · {log.action}</span><span className="text-gray-400">{formatDate(log.createdAt)}</span></div>)}</div>:<p className="text-gray-400 text-sm mt-3">Nessun ingresso registrato</p>}</DetailSection></aside></div>}
